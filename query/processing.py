...
from query.ranking_models import BooleanRankingModel, RankingModel,VectorRankingModel, IndexPreComputedVals
from index.structure import FileIndex, Index, TermOccurrence
...
class QueryRunner:
	...
	def get_relevance_per_query(self) -> Mapping[str, Set[int]]:
		...
		"""
		dic_relevance_docs = {}
		for arquiv in ["belo_horizonte", "sao_paulo", "irlanda"]:
			with open(f"relevant_docs/{arquiv}.dat") as arq:
				dic_relevance_docs[arquiv] = set(arq.readline().strip().split(","))
